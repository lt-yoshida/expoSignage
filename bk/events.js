...
async function getEvents() {
	...
	try {
		...
		for (let i = 0; i < enableIndex.length; i++) {
			...
			let eventEn = event.misc.enEvent;
			...
			for (let i = 0; i < sums.length; i++) {
				// let summary = "<b>" +  event.summary.replace("\n", "</b><br>");
				// sums[i].innerHTML = summary.replaceAll("\n", "<br>");
				let summary = event.summary;
				sums[i].textContent = summary;
			}

			const sumEns = document.getElementsByClassName('summary-en-' + targetIndex);
			for (let i = 0; i < sumEns.length; i++) {
				// let summaryEn = "<b>" + eventEn.summary.replace("\n", "</b><br>");
				// sumEns[i].innerHTML = summaryEn.replaceAll("\n", "<br>");
				let summaryEn = eventEn.summary;
				sumEns[i].textContent = summaryEn;
			}
